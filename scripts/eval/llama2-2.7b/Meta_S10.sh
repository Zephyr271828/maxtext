...
# export RUN_NAME="${MODEL_NAME}_Meta_S10_seqlen_${SEQ_LEN}_bs_${BATCH_SIZE}_grad_accum_${GRAD_ACCUM}_lr_${LR}_min_lr_ratio_${MIN_LR_RATIO}_warmup_ratio_${WARMUP_RATIO}"
# if [ ! -z "${TAG:-}" ]; then
#     export RUN_NAME="${RUN_NAME}_${TAG}"
# fi

CKPT_DIR=$(gsutil ls -d gs://${BUCKET_NAME}/model_ckpts/maxtext/${MODEL_NAME}_Meta_S10_seqlen_${SEQ_LEN}_bs_*_grad_accum_*_lr_${LR/e/*e}_min_lr_ratio_${MIN_LR_RATIO}_warmup_ratio_${WARMUP_RATIO}*/checkpoints/$(( NUM_STEPS - 1 )) )
...
CKPT_DIR=$(gsutil ls -d gs://${BUCKET_NAME}/model_ckpts/direct/${MODEL_NAME}_Meta_S10_seqlen_${SEQ_LEN}_bs_*_grad_accum_*_lr_${LR/e/*e}_min_lr_ratio_${MIN_LR_RATIO}_warmup_ratio_${WARMUP_RATIO}*/checkpoints/0 )
RUN_NAME=$(basename "$(dirname "$(dirname "$CKPT_DIR")")")
